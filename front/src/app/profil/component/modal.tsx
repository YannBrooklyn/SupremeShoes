...
export default function ModalProfil() {
    ...
    async function handlePutUser(event: any) {
        ...
        const idUser: number = jwt.decode(cookies.get("token")).id
    ...
    if (cookies.get("token")) {
                ...
                {cookies.get("token") ?
                    (
                        <form action="" onSubmit={handlePutUser} className="w-11/12 flex flex-col justify-center items-center h-96 lg:w-4/6" method="post">
                            ...
                            <button type="button" className="rounded-full font-xl h-8 w-auto h-auto"onClick={LogoutUser} style={{ color:"red", backgroundColor: "unset" }}>Se Deconnecter</button>
        ...
        router.push("/")
        return (
           <h3>Vous n'avez pas l'autorisation d'acceder à cette page.</h3>
        )
    }
}
